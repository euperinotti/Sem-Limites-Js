...
<head>
    ...
    <title>Meu Carrinho | Sem Limites</title>
...
<section class="cart-section">
        ...
        <div class="cl-header">
            ...
            <?php
                if(empty($_SESSION['carrinho'])) {
                    echo "<span class='cartmsg'>Seu carrinho está vazio :( </span>";
                } else {
                        foreach($_SESSION['carrinho'] as $pr => $qntd){
                            $stmt = $pdo->prepare("SELECT * FROM produto WHERE idproduto = :idproduto");
                            $stmt->bindParam(':idproduto', $pr);
                            $stmt->execute();

                            if($stmt->rowCount() > 0){
                                $produto = $stmt->fetch();
                                $subtotal += $produto['preco'] * $qntd;
            ...
            <div class="cl-pr-right">
                ...
                <div class="cl-pr-amount">
                    ...
                    <a href="cart.php?idprod=<?php echo $produto['idproduto'];?>&acc=remove" onclick="return confirm('TEM CERTEZA QUE DESEJA REMOVER ESTE ITEM DO SEU CARRINHO?')">X</a>
                </div>
            </div> 
            </div> 
            <?php }
            if($_GET['acc'] == "remove"){
                unset($_SESSION['carrinho'][$_GET['idprod']]);
    ...
    <div class="cart-right">
        ...
        <div class="cr-sumary-list">
            <span><strong>Produtos</strong></span> <span><?php echo $qntdprod = count($_SESSION['carrinho']);?></span> </br>
            <span><strong>Subtotal</strong></span> <span><?php echo "R$ " . $subtotal; ?></span> </br>
            <span><strong>Descontos</strong></span> <span><?php echo "R$ " . $desconto; ?></span> </br>
        </div>

        <div class="cr-sumary-total">
            <span>TOTAL</span> <span><?php echo "R$ " . $total;?></span> 
        </div>

        <a href="end-buy.php"><input type="button" value="FINALIZAR COMPRA"></a>
    </div>
</section>
